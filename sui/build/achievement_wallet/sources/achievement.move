module achievement_wallet::achievement {
    ...
    public entry fun mint_certificate_with_event(
        ...
        let achievement = Achievement {
            ...
            gps_data: gps_string,
            verified: true,
            sui_transaction_id: *tx_context::digest(ctx),
        };
    ...
    public entry fun batch_mint_clearances(
        ...
        let fee_coin = coin::split(&mut payment, developer_fee, ctx);
        let fee_balance = coin::into_balance(fee_coin);
        balance::join(&mut treasury.balance, fee_balance);
        
        // Always return remainder to organizer (must consume payment coin)
    ...
    public entry fun withdraw_from_treasury(
        ...
        let fee_balance = balance::split(&mut treasury.balance, amount);
        let coin = coin::from_balance<SUI>(fee_balance, ctx);
